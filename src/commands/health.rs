...
pub fn handle_health(config: &Config, system_info: &SystemInfo) {
    ...
    msg!("  Distribution: {} ({})", system_info.distro_name, system_info.distro_id);
    ...
    if check_command_exists("git") {
        ...
        msg!("  {} Git is not installed - required for repository management", red!("✗"));
    }
    ...
    if has_podman || has_docker {
        if has_podman && has_docker {
            msg!("  {} Container runtime found: podman and docker", green!("✓"));
        } else if has_podman {
        ...
        msg!("  {} No container runtime found - either podman or docker is required", red!("✗"));
    }
    ...
    for (name, repo_info) in &config.repositories {
        ...
        if repo_path.exists() {
            msg!("  {} Repository '{}' exists ({})", green!("✓"), name, repo_info.branch);
        } else {
            msg!("  {} Repository '{}' not found ({})", orange!("⚠"), name, repo_info.branch);
            msg!("    Run {} to clone repositories", style!("bold", "sourcery --update"));
        }
    }
...
mod tests {
    ...
    fn test_check_command_exists_with_invalid_command() {
        ...
        assert!(!check_command_exists("this_command_definitely_does_not_exist_12345"));
    }

...
pub fn handle_update_repo(config: &Config, _system_info: &SystemInfo) {
    ...
    for (name, repo_info) in repositories {
        ...
        if repo_path.exists() {
            ...
            if let Err(e) = clone_repository(&repo_info.url, &repositories_dir, name, &repo_info.branch) {
...
fn clone_repository(url: &str, target_dir: &Path, repo_name: &str, branch: &str) -> Result<(), Box<dyn std::error::Error>> {
